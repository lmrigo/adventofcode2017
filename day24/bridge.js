...
var day24Part2 = function () {
  ...
  for (var i = 0; i < input.length; i++) {
    ...
    while (nextStates.length > 0) {
      ...
      // if (steps++ % 10000 === 0) {
      //   console.log(steps, maxSize, maxStrength)
      // }
      var state = nextStates.pop()
      if (state.idxsUsed.length > maxSize) {
        maxSize = state.idxsUsed.length
        maxStrength = state.sum
      } else if (state.idxsUsed.length === maxSize
                  && state.sum > maxStrength) {
        maxStrength = state.sum
      }
      nextStates.push(...genNextStates(state))
    }
